---
title: "Oracle 인덱스 구조 원리 - row는 어디에 어떻게 저장되나"
date: 2026-05-14 12:00:00 +0900
categories: [Database]
tags: [database, oracle, index, b-tree, oltp]
---

> 샘플 OLTP 테이블(예시 `SAMPLE_TXN`)을 다루며 INSERT 한 row가 어디에 저장되고, 인덱스가 그 row를 어떻게 찾아가는지 따라간다. 모든 테이블/인덱스 이름은 학습용 예시 값이다. 실행계획 읽는 법은 별도 글 「Oracle 실행계획 읽는 법」 참고.

---

## 0. 큰 그림 — 이 글에서 다루는 내용

1. `INSERT` 한 row 가 저장되는 물리 저장 계층
2. heap table 기준 **ROWID** 구조
3. row 가 블록에 들어갈 때 **PCTFREE** 가 중요한 이유
4. 인덱스(B-tree)가 ROWID 를 저장하고 활용하는 방식
5. 인덱스를 만들 때 쓰는 **PGA 메모리**와 SGA 의 차이
6. 이런 설계가 **OLTP** 에서 특히 중요한 이유
...
```text
Database
       └─ Tablespace--------------------(논리 저장 단위. 예: USERS, APP_TS)
...
핵심 한 줄: **Oracle 의 논리 I/O 단위는 블록(block)** 이다. 원하는 row가 buffer cache에 없으면 그 row가 들어 있는 **블록 단위(예: 8KB)** 로 물리 I/O가 발생한다. 그래서 블록당 row 수가 성능에 큰 영향을 준다.
...
- **Row Directory**: 이 블록의 N번 row가 블록 내 몇 byte 지점에 있는지 담는 슬롯 배열. 인덱스가 가리키는 `RRR`(row number)이 이 슬롯 번호다.
- **Row Data**: 실제 row. 한 row 는 `row header + (컬럼길이 + 컬럼값) × 컬럼수` 형태.

### 2-2. ROWID — row 의 물리 주소

heap table 기준 `ROWID` 는 row의 물리적 위치를 가리키는 18자리(Extended ROWID) 주소다. row movement, segment 이동, IOT 같은 구조에서는 ROWID 안정성에 예외가 있을 수 있다.
...
**중요한 이유:** 인덱스는 키값과 함께 **이 ROWID 를 저장**한다. `INDEX RANGE SCAN → TABLE ACCESS BY INDEX ROWID` 가 실행계획에 2단계로 잡히는 이유가 이것:
...
```text
1. 세그먼트의 빈 블록 후보 목록에서 들어갈 블록을 고름
...
5. DML 수행 중 redo/undo 생성, commit 시 commit record 기록
```

여기서 **4번 = 인덱스가 N개면 INSERT 한 번에 트리 작업 N번** 의 출처가 나온다. 인덱스 추가에 신중해야 하는 이유다.
...
### 4-2. 비워두는 이유 — row migration / row chaining 방지

| 현상 | 언제 발생 | 결과 |
|---|---|---|
| **Row Migration** | UPDATE 로 row 가 커졌는데 그 블록에 자리가 없음 → row 를 **다른 블록으로 이사** | 원래 자리엔 새 주소 포인터만 남김. heap table에서는 인덱스 갱신을 피하려고 기존 ROWID를 유지한다. 단, 그 row 조회 시 **블록을 2번 읽음** (원래 블록 → 포인터 따라 새 블록) |
| **Row Chaining** | row 가 너무 커서 한 블록에 담기 어려움 | row piece가 여러 블록에 나뉘어 저장될 수 있다. LOB는 별도 세그먼트/out-of-line 저장 등 별도 규칙도 함께 봐야 한다 |
...
`PCTUSED` = 블록 사용량이 이 % **밑으로 떨어지면** 다시 INSERT 받을 후보 블록 목록에 복귀시키는 기준. Freelist 방식에서 의미가 있고, 현대 기본값인 ASSM에서는 Oracle이 비트맵으로 자동 관리하므로 보통 직접 다루지 않는다.

### 4-4. 실무 가이드

| 테이블 성격 | PCTFREE 권장 | 이유 |
|---|---|---|
| INSERT 후 거의 UPDATE 안 함 (로그성 이력) | **낮게** (예: 5 이하) | 빈 공간 예약 불필요 → 블록 꽉 채워 효율 ↑ |
| UPDATE 로 row 가 자주 커짐 (상태/값 변경 잦음) | **높게** (예: 20~30) | migration 방지 |

> 샘플 이력 테이블처럼 INSERT 후 `STATUS_CODE` 같은 **고정 크기 컬럼만 UPDATE** 하는 경우 → row 크기가 거의 안 변하므로 migration 위험이 낮다. PCTFREE를 낮게 잡을 수 있다.

---

## 5. 인덱스 구조 — B-tree와 leaf 연결 구조
...
Oracle 공식 문서는 B-tree 인덱스라고 부른다. 구조적으로는 leaf에 키와 ROWID가 있고 leaf block이 연결되어 있어 범위 스캔에 유리한 성질을 가진다.
...
| | 일반 B-tree | B+Tree 계열 구조 |
|---|---|---|
| 데이터 위치 | branch 노드에도 | **leaf 에만** |
| 범위/순차 스캔 | 불리 | **유리** (leaf linked list) |

자바 `TreeMap` 은 Red-Black tree(메모리 자료구조)라 결이 다르다. 디스크 인덱스는 한 블록에 키를 최대한 많이 담아 **트리 높이를 낮춰 I/O 횟수를 줄이는 것**이 목적이다.

### `CREATE INDEX` 내부 동작

```sql
CREATE INDEX IDX_SAMPLE_TENANT ON SAMPLE_TXN(TENANT_ID);
```

```text
1. 테이블 풀스캔 → 모든 row 의 (TENANT_ID, ROWID) 쌍 추출
...
| **Bitmap** | 카디널리티 낮은 컬럼. 읽기 위주 OLAP에 적합. DML 많은 OLTP에서는 보통 피함 |
...
|---|---|---|
| 공유 여부 | **모든 프로세스가 공유** | dedicated server 기준 **서버 프로세스 1개 전용 (비공유)** |
| 언제 생성 | 인스턴스 startup 시 1개 | 프로세스가 생길 때마다 1개씩 |
...
> 정렬/해시조인 작업판인 **Work Area** 는 PGA 소속이다.
> 이 구분이 「Oracle 실행계획 읽는 법」의 실행계획 저장 위치와 직접 연결된다.
...
```text
데이터가 work area 메모리에 다 들어감     → optimal  (메모리 내 완결)
조금 넘쳐서 temp tablespace 1회 왕복      → one-pass (디스크 1회)
많이 넘쳐서 temp 여러 번 왕복             → multi-pass(디스크 여러 번, 매우 느림)
```

→ 실행계획의 **Bytes** 컬럼이 의미 있는 이유: 옵티마이저는 추정 데이터 크기, workarea 정책, PGA 관련 파라미터, 동시성 등을 바탕으로 정렬/해시 작업 비용을 계산한다. `CREATE INDEX` 가 큰 테이블에서 느린 것도 정렬 단계가 work area 를 넘쳐 temp 로 spill 될 수 있기 때문이다.
...
| 예시 | 주문, 결제, 상태 변경 | 일별 통계 배치, 리포트 |
| 핵심 관심사 | **응답속도 + 동시성** | **처리량(throughput)** |

OLTP API 는 보통 한 사용자의 한 건을 인덱스로 좁혀 조회하고, 짧은 트랜잭션이 동시에 많이 실행된다.

### 7-2. OLTP 에서 앞 내용들이 갖는 의미

| 앞에서 본 것 | OLTP 에 미치는 영향 |
|---|---|
| **블록이 I/O 최소 단위** | 단건 조회도 블록 단위로 읽는다. 불필요한 블록 접근을 줄이도록 인덱스를 설계해야 동시성 유지 |
...
| **인덱스 N개 = INSERT 트리작업 N번** | INSERT 많은 테이블에서 인덱스 남발 = 쓰기 지연. **꼭 필요한 인덱스만** |
| **PGA work area** | OLTP 는 큰 정렬/해시가 드물어야 정상. 불필요한 `ORDER BY`/`DISTINCT` 가 끼면 세션마다 PGA 소모 → 동시성 저하 |

### 7-3. Bitmap 인덱스를 OLTP 에서 피하는 이유

- Bitmap 인덱스는 **한 키값이 다수 row 를 비트맵으로 커버**한다.
- 한 row 의 해당 컬럼을 DML(INSERT/UPDATE/DELETE) 하면 → 그 **비트맵 세그먼트 조각 전체에 락**이 걸린다.
- OLTP 는 같은 영역에 동시 DML 이 많을 수 있는데, 이 락이 **동시 트랜잭션을 직렬화** → 동시성 저하.
- 반면 OLAP 은 읽기 위주 + DML 이 배치성이라 이 락이 문제 안 됨 → bitmap 이 오히려 유리(압축·집계 강점).

→ 결론: DML이 많은 OLTP 테이블에서는 일반적으로 Bitmap 인덱스를 피하고 B-tree 인덱스를 우선 검토한다 (예: `UK_SAMPLE_BUSINESS_KEY`, `IDX_SAMPLE_TENANT_STATUS`).
...
| ROWID 구조 | heap table 기준 `Object(6) + File(3) + Block(6) + Row(3)` = row 위치. 인덱스가 이걸 저장 |
...
| B-tree 인덱스 | 데이터는 leaf 에 있고, leaf 끼리 linked list → 범위검색에 유리. 높이 균형 |
| SGA vs PGA | SGA=공유(Buffer Cache, Library Cache). PGA=프로세스 전용 영역(정렬/해시 work area) |
| OLTP 함의 | 핀포인트 인덱스 설계 + 인덱스 최소화 + bitmap 지양 + 큰 정렬 회피 |
